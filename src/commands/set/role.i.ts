...
export default async function run({
  ...
  if (member.id === client.user.id) {
    const AssignEmbed = new ErrorEmbed({
      user,
      code: 403,
      error: "Cannot assign a role to a the yourself",
    });
    try {
  ...
  if (member.roles.cache.has(role.id)) {
    const AssignEmbed = new ErrorEmbed({
      user,
      code: 403,
      error: "Cannot assign a role to a person who already has the role",
    });
    try {
      return interaction.reply({
        embeds: [AssignEmbed],
  ...
  member.roles
    ...
    .catch((e) => {
      const RoleBelowEmbed = new ErrorEmbed({
        ...
        code: 403,
        error:
          "Insufficient Permissions. [ Bot ]`\nValidation: `Place the bot role higher than the role adding",
      });
      console.log(e);
